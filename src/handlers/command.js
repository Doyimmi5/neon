...
module.exports = {
  ...
  handlePrefixCommand: async function (message, cmd, settings) {
    ...
    if (cmd.category === "OWNER" && !OWNER_IDS.includes(message.author.id)) {
      return message.safeReply("Este comando é acessível apenas aos proprietários do bot");
    }

    // Premium commands
    if (cmd.isPremium) {
      const hasPremium = await isPremiumUser(message.author.id);
      if (!hasPremium) {
        return message.safeReply("Este comando requer premium! Use `/premium` para mais informações.");
      }
    }
  ...
  handleSlashCommand: async function (interaction) {
    ...
    if (cmd.category === "OWNER" && !OWNER_IDS.includes(interaction.user.id)) {
      return interaction.reply({
        content: `Este comando é acessível apenas aos proprietários do bot`,
        ephemeral: true,
      });
    }
    ...
    // Premium commands
    if (cmd.isPremium) {
      const hasPremium = await isPremiumUser(interaction.user.id);
      if (!hasPremium) {
        return interaction.reply({
          content: "Este comando requer premium! Use `/premium` para mais informações.",
          ephemeral: true,
        });
      }
    }
