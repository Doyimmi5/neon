...
module.exports = {
  name: "meme",
  description: "obter um meme aleatório",
  category: "FUN",
  isPremium: true,
...
async function getRandomEmbed(choice) {
  ...
  if (!response.success || !response.data) {
    return new EmbedBuilder().setColor(EMBED_COLORS.ERROR).setDescription("Falha ao buscar meme. Tente novamente!");
  }

  const json = response.data;
  if (!json.postLink || !json.url || !json.title || !json.ups) {
    return new EmbedBuilder().setColor(EMBED_COLORS.ERROR).setDescription(`Nenhum meme encontrado para ${choice}`);
  }

  // Check if the meme is marked as NSFW
  if (json.nsfw === true) {
    return new EmbedBuilder().setColor(EMBED_COLORS.ERROR).setDescription("Este meme contém conteúdo NSFW");
  }
