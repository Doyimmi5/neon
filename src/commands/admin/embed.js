...
module.exports = {
  ...
  async messageRun(message, args) {
    ...
    if (!channel.canSendEmbeds()) {
      return message.reply("Não tenho permissão para enviar embeds neste canal");
    }
    message.reply(`Configuração de embed iniciada em ${channel}`);
  ...
  async interactionRun(interaction) {
    ...
    if (!channel.canSendEmbeds()) {
      return interaction.followUp("Não tenho permissão para enviar embeds neste canal");
    }
    interaction.followUp(`Configuração de embed iniciada em ${channel}`);
...
async function embedSetup(channel, member) {
  ...
  if (!modal) return sentMsg.edit({ content: "Nenhuma resposta recebida, cancelando configuração", components: [] });
  ...
  const buttonRow = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId("EMBED_FIELD_DONE").setLabel("Concluir").setStyle(ButtonStyle.Primary)
  );

  await sentMsg.edit({
    content: "Por favor, adicione campos usando os botões abaixo. Clique em concluir quando terminar.",
    embeds: [embed],
