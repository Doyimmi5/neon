...
module.exports = {
  name: "help",
  description: "menu de ajuda dos comandos",
  category: "UTILITY",
...
async function getHelpMenu({ client, guild }) {
  ...
  for (const [k, v] of Object.entries(CommandCategory)) {
    ...
    options.push({
      label: v.name,
      value: k,
      description: `Ver comandos na categoria ${v.name}`,
      emoji: v.emoji,
    });
  }
  ...
  const menuRow = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .setPlaceholder("Escolha a categoria de comandos")
      .addOptions(options)
  );
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      "**Sobre Mim:**\n" +
        `Olá, eu sou ${guild.members.me.displayName}!\n` +
        "Um bot Discord multifuncional legal que pode atender todas as suas necessidades\n\n" +
        `**Me Convide:** [Aqui](${client.getInvite()})\n` +
        `**Servidor de Suporte:** [Entrar](${SUPPORT_SERVER})`
    );
